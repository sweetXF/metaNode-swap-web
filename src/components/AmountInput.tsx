...
export const AmountInput = ({
    ...
    const tokenInfo=token ? tokenMap.get(token.address.toLowerCase()) : null;
    const symbol=tokenInfo?.symbol ?? token?.symbol ?? 'Token';
    const decimals=tokenInfo?.decimals ?? 18;
    const balanceRaw=tokenInfo?.balance;

    const balanceStr=balanceRaw !== undefined ? Number(formatUnits(balanceRaw,decimals)).toLocaleString("en-US",{ maximumFractionDigits: 2 }) : "0";
